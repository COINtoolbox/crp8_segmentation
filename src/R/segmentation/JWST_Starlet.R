...
fits_path <- "/Users/rd23aag/Documents/GitHub/crp8_segmentation/data/raw/datacube_reg2.fits"
...
ny   <- dim(cube)[2]
nlam <- dim(cube)[3]
collapsed <- collapse_white_light(cube, kclip = 5, use_weights = FALSE)

image(stretch_asinh(collapsed), col = viridis::turbo(256))
...
mask_rec <-  rec_all > 0  # logical mask
image(mask_rec)
...
pal_beach <- colorRampPalette(c(
   "red",
  "#255e65",  # deep greenish-cyan (sea shadow)
  "#3591a0",  # sea turquoise
  "#d9bea1",  # sand
  "#fff2d5"   # bright sunlit highlight
))(256)
...
## Optionally: all scales together, if you want a reference panel
# rec_all <- starlet_reconstruct(
#   dec,
#   keep_scales    = 1:J,
#   include_coarse = FALSE,
...
#   mode           = "soft"
# )
# df_list[[length(df_list) + 1]] <- mat_to_df(rec_all, "All scales (1…J)")

df_all <- do.call(rbind, df_list)

stretch_asinh <- function(x, Q = 5) {
  y <- x
  y[y <= 0] <- NA_real_
  asinh(y / Q)
}
...
pdf("starlet_2d_sagui7.pdf",height = 6.5,width = 8.5)
...
## ============================================================
## 3. Binary mask from reconstruction + write to FITS
## ============================================================
...
cube_cap <- list(imDat = cube_na)
seg_cap  <- capivara::segment(cube_cap, N = 25)
seg_guara <- guara_hclust_sample_assign(cube_cap$imDat, N = 25)


image(seg_guara$labels_img,col=c("white",inferno(25)))
image(seg_cap$cluster_map,col=c("white",inferno(25)))


plot_cluster(seg_guara$labels_img,"turbo")
